...
- **AI 易错点**【多开窗口共用一份真实画面尺寸；后布局的窗口会把别的窗口压窄】**每个托管会话只有一份真实画面**，所有 corral 窗口都是在看/改同一扇窗。A 窗口两格分屏时每格按约 1/2 铺；B 窗口打开「活跃会话 / Active sessions」若一次铺了四格（或更多会话均分），会按约 1/4 去改那两份画面。A 窗口格子仍是两格各一半，但格子里的助手已经按四分之一宽重排——观感就是「两格却各只占 1/2 的 1/2」。**不要当成单窗口分屏把宽度除了两次。** 先问有没有另一扇 corral 开着活跃会话或更多格。手机端已裁定不许改电脑画面尺寸（见远程知识库）；桌面多窗口目前仍是「谁后改尺寸谁赢」，2026-08-29 机主实报，产品上尚未改成「较窄的观看方不许压窄」。消歧：外层缩放也不自愈、画面钉在约 40 列那种是创建期窄宽（维护指南 2026-08-14 条），不是这条。
- **AI 易错点**【Cursor 画面宽度抽动 / 有的会话抖有的不抖】**不是格子自己在 1 列里抖。** 同一条托管会话被两扇窗口按不同格数看着时，两边都会把真实画面改成自己的宽度：现场 `events.log` 里同一 `corral-cursor-*` 在约 165 列（单格）和约 82 列（两格一半）之间来回 `host_size_drift`（2026-08-29 19:22–19:33）。Cursor 每次被改宽度都会整屏重排、行折来折去，观感就是「宽度疯狂抽动」。**只被一扇窗口看着的会话不会走这条路径，所以有的抖、有的不抖。** 旧实现对齐成功后只要真实宽度再被改走就会再抢回来，两扇窗口永远互抢；现改为：本格见过「真实尺寸 == 我的期望」之后，期望没变就让出，只有自己改了分屏/窗口尺寸才重新对齐。回归：`test_host_size_heal_yields_after_another_window_wins`。**禁止**把这条当成「抓帧采到了中间态」再给抓帧加稳定过滤——那种滤法对 Cursor 自发整屏重画已经验证无效（见下条）。叠加上面这条互抢之后，Cursor 自己的长对话整屏重画仍会让少数会话自己跳，那是上游限制，不是宽度互抢。
