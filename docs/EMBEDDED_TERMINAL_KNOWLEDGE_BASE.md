...
- **输入蒙版**：焦点在侧边栏且右栏是活着的实时会话时，该格整体压暗（`EmbedPane.input_masked`）并在底条写明“当前输入不会进入这里”，避免用户对着没接管的画面空打。焦点在任一格内时不压暗任何格。压暗系数在 `embed_pane._MASK_FG_FACTOR` / `_MASK_BG_FACTOR`（越大越接近原色；当前约 0.63 / 0.71，相对早期再轻约两成）。
