...
- **输入转发是黑名单不是白名单**：右栏持焦时，corral **只拦截壳层键**（`Ctrl+\` 回列表、有选区时的 `Ctrl+C` 复制、主界面高优先级的 `Ctrl+F` 全文搜索 / `Ctrl+P` 置顶 / `Ctrl+N` 新建 / `Ctrl+A` 高级操作等），其余按键一律尽量译成 tmux 键名后放行；译不出但仍有 `event.character`（含 `Ctrl+_` 的 `\x1f` 等控制字节）时走 `send_literal` 兜底。禁止再写成「只转发 Ctrl+字母」——那会让 `Ctrl+/`（多数终端 ≡ `Ctrl+_`，Claude Code 撤销输入）等静默丢失。**不要拦截 `Ctrl+Shift+B` / `Ctrl+B`**：侧栏显隐没有快捷键，只点顶栏 ◀/▶。回归：`TranslateTextualKeyTests`。
...
- **AI 易错点**【输入转发是黑名单】**右栏持焦时只拦壳层快捷键，其余一律放行**。`translate_textual_key` 必须覆盖 `Ctrl+_` / `Ctrl+/`（→ `C-_`）、带修饰方向键、`Alt+字母` 等；译不出时 EmbedPane 还要用 `event.character`（含非打印控制字节）`send_literal` 兜底。禁止退回「只转发 `Ctrl+字母`」白名单——真机事故：内嵌 Claude Code 里 `Ctrl+/` 撤销输入无效。壳层键清单：`Ctrl+\`、有选区的 `Ctrl+C`、主界面 `Ctrl+F` / `Ctrl+P` / `Ctrl+N` / `Ctrl+A`（priority）。**不要拦截 `Ctrl+Shift+B` / `Ctrl+B`**（侧栏显隐没有快捷键）。回归：`TranslateTextualKeyTests`。
