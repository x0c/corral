...
- **AI 易错点**【活跃会话首帧禁止消息预览】只要格子绑定了托管运行时，生产路径就不得把消息预览渲染器传入实时画面；首帧未到时保持运行时底色空白或直接显示已缓存的运行时画面，**绝不闪现对话预览**。消息预览只属于未托管、已结束或“在别的窗口运行”的会话。回归：`test_active_pane_never_receives_a_message_preview_renderer`。
