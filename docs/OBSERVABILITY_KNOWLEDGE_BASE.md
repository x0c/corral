...
flowchart TD
    ...
    B --> C[corral diagnose 读取本机路径与环境事实]
...
| `host_size_drift` | 抓帧线程发现 tmux 真实尺寸落后于**有效尺寸**（各观看方登记的最大宽）并重发 `resize` | 实际/期望宽高与重试次数；同一目标最多 3 次、间隔约 2s。有效尺寸由 `desired_host_size` 取最宽仍存活观看方；较窄窗口应 crop 而不是把共享窗压窄。**同一会话在约 2 倍宽度之间来回（如 165↔82）= 观看方登记丢失或 `window-size latest` 把窗打回 80 列**，不是单次 resize 失败 |
