...
开发机 `pair` 载荷里的 `l`（local hints）与 `r`（relay）都要填对；若旧配对二维码没有 `r=`，手机可在后续 `hello` 里读到 `relay_url` / `relay_enabled` / `local_enabled`（未配对也返回）并写回本地 Host 记录，无需重新扫码。**开源默认不内置任何共享中继**：新装 `relay_enabled=false`、无 `relay_url`，只靠局域网；换网必须自建中继并写入 `r=` / `--relay-url`（见文首硬规则）。
