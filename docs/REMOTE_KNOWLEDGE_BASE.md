...
- 手机发出的话必须出现在正在看的对话里，助手回复必须经实时订阅到达。发送成功只表示已经贴进正在跑的窗口，**不等于**对话气泡已更新：气泡来自助手事后写下的历史，不是发送回执。点发送后立刻清空输入框，不能拿「输入框空了」当对话已跟上。
- 新开会话会先用一张临时卡片，助手落下第一句真实历史后再换成正式编号。电脑侧栏会跟着换；**手机仍停在旧编号的详情页时，必须继续能发、能收**。禁止把旧编号当成「已经不在列表里」——那不是配对错了，是编号转正后远程没跟上。排查「发了消息对话不更新 / 看不到回复 / This session is no longer in the list」进下文踩坑。
...
| 现象 | 原因 / 处理 |
|---|---|
| 手机发得出话、对话气泡不出现、看不到助手回复，再发送提示 `This session is no longer in the list` / 「这条会话已经不在列表里了」 | **不是配对身份错了**：第一条已经进正在跑的窗口，说明通道和窗格都找对了。发送是往窗口里粘贴；对话气泡是另读助手历史。新开会话的临时卡片换成正式编号后，电脑侧栏会迁键，手机详情还拿着旧键，于是发得出、历史对不上、再发就报不在列表里。远程必须把旧键认到正式会话，并把正在看的对话订阅改读新历史、事件仍推到手机原来的通道。禁止改成「让用户退回列表重进」或把这条报错当成配对失败。自 CLI **0.24.154** 起服务端跟随转正；旧常驻进程未重启仍会复现 |
