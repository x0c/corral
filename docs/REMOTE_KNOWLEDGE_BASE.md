...
已知缺口（按用户能感知的后果；**未在本轮修复**，改之前先读本节，不要用「给中继加账号」冒充端到端）：

1. **自建/个人公网中继：电脑离线时，知道路由标识的人可以占住中继坑位。** 单租户默认「第一次见到就信、离线允许换注册钥匙」。攻击者不必解开会话：只要电脑合盖/掉线，用另一把注册钥匙连上同一路由标识，真实电脑再上线会被当成钥匙不匹配而进不去。手机仍用扫码钉死的开发机公钥握手，对占坑者解不开，表现为「蜂窝连不上、家里局域网还能用」。路由标识会出现在配对二维码和推送明文壳的 `host_id` 里，不是秘密。多租户登记路径没有这条；当前默认个人公网中继是单租户。中继 README「What this does not yet protect」已预告要改成配对时签发短时票据。
2. **配对码宣称一次性，并发扫同一张码可以两台都成功。** 读码与作废不是原子操作。肩窥 + 同时提交，或两部手机对着同一终端扫，会配上两台都有完整控制权的设备。
3. **中继对手机接入只认路由标识，不先验设备身份。** 未配对者读不到、也控制不了开发机，但能消耗有限的通道与握手算力。中继按来源 IP 限流（每 IP 每分钟约 30 次）；推荐的 Caddy 反代不把真实手机 IP 传给中继，限流会变成「整台反代共用一桶」——一个人就能把所有手机的公网接入打满。开发机自己还有通道上限，不会被灌到线程耗尽。
4. **扫码界面没有「这等于把电脑交给这部手机」的明示。** 根凭据的产品提示只在电脑端文档/隐私说明里，手机扫码文案只说对准二维码。
5. **局域网口听在全部网卡。** 给 Tailscale / 同网直连用；若这台机器有公网地址或端口转发，这个口会暴露到互联网。未配对仍要猜 80 bit 配对码；已配对设备才能握手。不要把它理解成「只绑了家里 Wi-Fi」。
6. **每次 `corral remote start`（含已在运行时刷新二维码）都会重新打开十分钟配对窗口。** 终端回滚、SSH 录像、把配对链接发到聊天里，等于把根凭据交出去。这是产品设计，不是实现漏洞。

不是漏洞、不要当漏洞修：

- 中继能看见谁在连谁、每帧多大、何时连——零知识中继的固有元数据，不是「加密失败」。
- 未做中继证书钉扎：系统信任链被劫持时攻击者最多变成另一台中继（看元数据、可断线），解不开正文。
- 已配对手机能向助手粘贴任意文本 = 完整执行权。这是信任边界，不是要加一层「手机沙箱」就能消掉的。丢失手机用 `corral remote unpair`，不要指望中继替你撤销。
