...
- **会话结束后的手机系统通知（产品方向 · 2026-09-15）**：用户离开前台后，一轮正常结束或异常结束（额度用尽、限流、供应商报错等）都应能发系统通知。触发与文案口径**必须基于 SessKit 的会话结束状态能力**（`status_tag` 已完成 / 已中断 + `last_agent_msg` 摘要；合同见 `~/Codes/SessKit/docs/CONTRACT.md`「Status tags and abnormal endings」，Corral 钉 SessKit ≥0.1.5）。禁止只看进程退出、侧栏关注圆点、或裸 `DONE`/`已完成`（假完成会把额度挂掉当成干完了）。成功完成与异常结束要区分文案；异常通知应带可读报错摘要。实现层（何时盯、去重、APNs 密封盒）仍走本文件推送链路；状态真源不另造一套。扫描侧细则见 `SESSION_SCANNING_KNOWLEDGE_BASE.md` §6。
