...
**Remediation implemented, acceptance incomplete (2026-09-12):** local fallback ranking, handoff/command stripping, generation prompt, and cache acceptance were changed. The independent implementation review below found reproducible gaps; do not interpret the historical "delivered" checklist as complete title-quality acceptance. Confirmed-valid cached successes still stay stable; selected invalid cached strings (bare command labels, empty-session placeholders, context-free `实现`/`执行`/`Task`, truncated handoff wrappers) are no longer treated as successes. The investigation below remains the evidence record.

## Independent implementation review (2026-09-12)

Scope: title changes included in `b9562eb`, reviewed at `a1562d8`, installed command 0.24.194 loading this checkout. No product code, credentials, or real title cache were changed by this review. Code and native-history evidence are local; no external model behavior is inferred.

1. **P1 — Handoff context extraction runs after the context was truncated.** A fresh installed-registry scan of the three original Claude/Codex handoff samples supplied exactly 300 characters of `first_user_msg`. Their raw JSONL records still contain the conversation digest, but `_prompt_item` receives only the wrapper prefix and produces `user_request` equal to `实现`, `说了几百遍了`, or `你审查一下`. The new test feeds a complete `Handoff.render_prompt()` directly into title code, bypassing this real boundary. Extract bounded task context before truncation, or enrich it in the background; verify native history -> scanner -> actual title request. Increasing the title function's slice to 400 cannot recover bytes already lost upstream. Even with the full wrapper, `_split_handoff_text` includes the explanatory digest heading, making the local title `Below is a conversation ex…`; remove framing before selecting the first content line.
2. **P2 — The new eight-character clause heuristic deletes real tasks.** `_compact_title('修复登录失败，不要改动其他功能')` returns `不要改动其他功能`; `_compact_title('修复闪退，保留现有界面布局')` returns `保留现有界面布局`. The newly added loop discards every short leading clause, not just emotional text. Keep short substantive requests; do not use clause length as a proxy for emotional content.
3. **P2 — Insufficient-context retries track file size instead of model input.** With an insufficient result cached for a session, changing only `size_bytes` from 100 to 200 changes `needs_generation` from false to true while `_prompt_item` remains identical. Tool/log growth can therefore requeue unchanged insufficient inputs; the daemon rescans after each round and can submit again during ongoing output. Conversely equal-size input changes are not detected. Fingerprint the actual bounded title input and apply a bounded retry schedule, rather than treating any history-byte change as new task information.
4. **P2 — Pure insults still pass successful-title acceptance.** `_compact_title('你他妈的')` returns the original insult because removing the prefix yields an empty string and `_strip_emotion_prefix` restores it. `_is_low_value_title` does not reject it. Both a supplied cached success and a mocked model result `你他妈的` are accepted as a stable final title despite a clear login-fix request. Verify rejection on fallback, model-result, and existing-success paths; removing an empty emotional prefix must not restore it as meaningful text.
5. **P2 — Invalid-result diagnostics are lost before persistence.** A model result containing only `Task` is invalid, but `generate_chunk` discards its raw response and returns `{}`. `persist_chunk` consequently records `failure_reason=transport`, not `invalid`. Malformed JSON follows the same collapsed path. Preserve the failure category across generation and persistence; otherwise later diagnosis again attributes content failures to transport.

Validation: all 17 new title tests passed. All five shared-title tests passed with `CORRAL_ISOLATE_MANAGED_HOSTS=1`; one initially failed without that isolation because real managed sessions affect the store. Additional read-only deterministic probes reproduced findings 2–5; a fresh real scan and native JSONL inspection established finding 1. No real model-generation quality claim is made: the installed environment still has no configured Corral gateway key. Successful local fallback repair does not validate the changed model prompt or its insufficient-context behavior end to end.
