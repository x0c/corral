...
4. 会话编号在新开后可能从临时卡片换成正式编号。正在打开的详情必须继续用手机手里的旧编号发输入、收对话事件；开发机把旧编号解析到正式会话，并把历史读取切过去，事件仍推到原来的通道。停止、删除、恢复、置顶等写操作也必须改正式编号，不能写到已经退役的临时卡片。不能要求手机先知道新编号，也不能因此报「已经不在列表里」。发送成功不等于对话气泡已在；气泡仍来自历史订阅。
