...
4. 服务端发送队列必须有界并按优先级背压：控制和输入优先，实时事件可合并，画面丢旧保新，历史页暂停或取消。队列满不能无条件踢掉整条设备连接。
