...
6. **高级操作只表达业务选择。** 选择同一助手的文案与结果是原生恢复；选择其他助手是“读取来源历史后新建会话”。默认优先选择第一个可用的其他助手，没有可用目标时才回到来源助手。**界面内嵌时，跨助手接力默认在被接力会话旁加一格分屏**（与顶栏“加助手”同路径）；同助手原生恢复仍聚焦原会话。满格时提示上限，不静默覆盖。
...
3. 再选择一个已安装的其他助手，确认目标以新会话启动，并收到源历史路径、格式提示及“跨助手接力”说明；**界面内嵌时右栏应与被接力会话并排分屏**（源会话仍在，不得整屏换成新会话）。
