...
- **高级操作**：用户对既有会话选择目标助手的业务入口；**无论选同一助手还是其他助手，一律读取源历史后新建会话**（同助手另起用于原会话卡住 / 出 bug）。真正的原生恢复走侧边栏回车，不走高级操作。
...
6. **高级操作一律新建。** 弹窗里每个助手（含来源自身）的文案都是“读取来源历史后新建会话”；同助手另起给原会话卡住 / 出 bug 用。默认优先选择第一个可用的其他助手，没有可用目标时才回到来源助手。真正的原生恢复只走侧边栏回车。**界面内嵌时，接力新建默认在被接力会话旁加一格分屏**（与顶栏“加助手”同路径）。满格时提示上限，不静默覆盖。
