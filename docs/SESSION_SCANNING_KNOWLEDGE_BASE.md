...
> **2026-09-10**：本机助手历史的格式解析与统一 transcript **真源在开源库 SessKit**（`sesskit` 包 / `~/Codes/SessKit`）。Corral 的 `cli/src/corral/scan/` 是同模块别名，产品层（TUI、判活编排、接力、远程）仍在本仓。改解析口径先改 SessKit。
