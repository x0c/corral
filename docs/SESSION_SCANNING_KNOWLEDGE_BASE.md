...
| Pi | 打开的 jsonl；进程内记住的当前文件；命令行 `--session` / `--session-id`；完整 `PICKUP_SESSION_ID`；其余 TUI 按「进程启动 ≤ 会话创建」一对一认领 | 禁止再按「同 cwd 仅最新一条」猜测。进程内 `/new` 换文件后，打开的 jsonl（及本进程记住的那次切换）优先于启动时钉死的 `--session-id` / `PICKUP_SESSION_ID`。`-c`/`--continue` 才回落到该目录未标记的最新一条。`-p` 打印模式与 `auth`/`install` 等子命令不算 TUI。位置参数里的接力说明词不当 argv | 无法探测时返回空映射 |
...
| 修改 Pi 扫描或预览 | Pi 扫描器 | `scan.pi.scan_sessions()`、`_apply_live_flags()`、`load_conversation()` | JSONL 首行必须是 session；从最新叶子沿 `parentId` 回溯；判活按 ① 打开的 jsonl ② 本进程记住的「该 pid 上次在写哪条」（**且该记忆的目标会话必须晚于进程启动才落盘，早于进程创建的必是写错的记忆，信任前剔除并回写磁盘**）③ `--session` / `--session-id` ④ 精确 `PICKUP_SESSION_ID` ⑤ `-c` 才回落到该目录未标记的最新一条 ⑥ 其余 TUI 按「进程启动 ≤ 会话创建」一对一认领，禁止「同 cwd 仅最新一条」；进程内 `/new` 后启动 ident 不得压过当前 jsonl；写字节/mtime 相关性认领**不得抢已绑在别的活进程上的会话**；`-p`/`auth`/`install` 不算 TUI；`live_processes("pi")` 需 cmdline 兜底（comm 常是 `node`） |
