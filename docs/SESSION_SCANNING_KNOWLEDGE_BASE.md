...
- **AI 易错点**【禁止】用 SessKit `status_tag=已完成` 单独当作「活儿干完了可推送」→ 额度用尽 / 限流 / 供应商报错在原生历史上常仍带“完成”类标记，但必须算异常结束。权威口径与 2026-09-15 实机缺口（Codex usage limit 被标已完成、Pi 429 的 `errorMessage` 从 `load_events` 丢失）见 `~/Codes/SessKit/docs/CONTRACT.md`「Status tags and abnormal endings」。修解析只改 SessKit；Corral 通知层在缺口未关前不得只信 `DONE`。
