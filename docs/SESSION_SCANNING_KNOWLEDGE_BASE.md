...
| 修改 Pi 扫描或预览 | Pi 扫描器 | `scan.pi.scan_sessions()`、`_apply_live_flags()`、`load_conversation()` | JSONL 首行必须是 session；从最新叶子沿 `parentId` 回溯；判活按 ① 打开的 jsonl ② 本进程记住的「该 pid 上次在写哪条」③ `--session` / `--session-id` ④ 精确 `PICKUP_SESSION_ID` ⑤ `-c` 才回落到该目录未标记的最新一条 ⑥ 其余 TUI 按「进程启动 ≤ 会话创建」一对一认领，禁止「同 cwd 仅最新一条」；进程内 `/new` 后启动 ident 不得压过当前 jsonl；`-p`/`auth`/`install` 不算 TUI；`live_processes("pi")` 需 cmdline 兜底（comm 常是 `node`） |
