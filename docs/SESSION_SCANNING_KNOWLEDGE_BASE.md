...
- **AI 易错点**【禁止】用 SessKit `status_tag=已完成` 单独当作「活儿干完了可推送」→ 额度用尽 / 限流 / 供应商报错在原生历史上常仍带“完成”类标记，必须看是否 `已中断` 以及 `last_agent_msg` 是否其实是报错摘要。SessKit ≥0.1.5 已把 Codex usage-limit / Pi 429 标成中断并保留原文；口径见 `~/Codes/SessKit/docs/CONTRACT.md`「Status tags and abnormal endings」。Corral 通知层仍应区分成功完成与异常结束，不得只信 `DONE`。
