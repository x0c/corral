...
- **AI 易错点**【禁止】`SessionStore.refresh` 在签名命中时无条件 `_merge_live_state`、跳过「内存键集合 == 本轮扫描键」校验 → 乐观删除清空内存后若磁盘删除失败，侧栏会空着不恢复；集合变化时必须完整 `_merge_scanned`（事件应走 `reason=refresh` 而非 `refresh_live`）。
