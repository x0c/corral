...
- **排查「侧栏莫名出现 cli 生成 s0 / Claude 点开没有消息 / 空白会话我不认识」：先不要当成标题生成又去调了 Claude。** 2026-09-12 本机：真实标题缓存里有测试键 `claude:s0`…`s29`（标题 `生成s0` / `标题5`），正在跑的 TUI 又认领了保活通道里界面测试留下的空壳（cwd 常是 `cli/` 仓库，无历史路径、无用户消息）。列表会显示「cli 生成 s0」，点开当然没有对话。现网关路径不会为生成标题拉起 Claude；旧 CLI 噪音靠 `PROMPT_MARKER` 事后过滤，从来不是单独房间。反向污染与认领见 [会话扫描知识库](SESSION_SCANNING_KNOWLEDGE_BASE.md) §6、[终端界面知识库](TERMINAL_UI_KNOWLEDGE_BASE.md) §6。
