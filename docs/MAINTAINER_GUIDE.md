...
- **tmux 配置内容内联在 `keepalive.py` 的 `_tmux_config()` 里，不是仓库里一个独立的 `.conf` 文件**——这是踩过坑之后改的：独立配置曾未进入安装产物，源码目录能跑、真实安装后却找不到。改成 `_ensure_config_file()` 在每次 `wrap_plan` / `host_session` 时把内联字符串落盘到 `~/.cache/corral/keepalive.tmux.conf`（内容变了才重写，同 `titles.py` 缓存目录）。`default-terminal` 按本机 terminfo 在 `tmux-256color` → `screen-256color` → `xterm-256color` → `screen` 中择优（Ubuntu 最小镜像常缺 `tmux-256color`，硬写会导致 `new-session -d` 失败、TUI 卡在「Press Enter to restart」静态页）。**生成 tmux 子进程 / `exec` wrap_plan 时必须走 `keepalive.tmux_env()`（剥掉 `LD_LIBRARY_PATH`）**：GitHub `actions/setup-python` 在 Linux 上会把解释器 lib 目录写进 `LD_LIBRARY_PATH`，系统 tmux 因此可能链到错配的 `libtinfo`/ncurses，立刻以 `server exited unexpectedly` 退出（2026-09-11：同提交 macOS 冒烟绿、Ubuntu 冒烟红）。禁止为「消掉 CI 红」在 workflow 里 `unset LD_LIBRARY_PATH` 再跑冒烟——那会把产品侧回归藏到托管 UI 超时里。改配置内容只改 `_tmux_config()`，不要再新建独立文件；改完实际安装 wheel 到临时目录，确认没有引入新的包外文件依赖。
