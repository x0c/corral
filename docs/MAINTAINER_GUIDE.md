...
- 判活（`scan.pi._apply_live_flags`）正向证据优先：打开的 jsonl（有隔离目录时还须在该目录内）→ `corral-<ident>/` 隔离目录里 mtime 最新的一条 → 本进程记住的「该 pid 上次在写哪条」→ `--session` / `--session-id` → 精确等于会话 id 的 `CORRAL_SESSION_ID`；未命中时 `-c`/`--continue` 回落到该目录未标记的最新一条，其余裸 TUI 按「进程启动 ≤ 会话创建」一对一认领。禁止再按「同 cwd 只留最新一条」猜测。带隔离目录的进程禁止认领目录外文件，也禁止把默认堆新文件认成自己的 `/new`。指向 Pi 默认 cwd 堆的 `--session-dir` 不算隔离，以免把 `--session` 恢复改绑到堆里别人的新文件。裸 `pi` 不长期持有 jsonl、命令行也不带会话参数，旧实现会把仍在跑的会话标成已结束。**进程内 `/new` 会换 uuidv7 文件，启动 ident 不得压过当前 jsonl**，否则侧栏标题停在旧卡、新历史被标成 Ended。`-p` 打印模式与 `auth`/`install` 等子命令不算 TUI。npm 包装后进程 comm 是 `node`，`pgrep -x pi` 不够，必须 cmdline 兜底；`node …/cli.js` 的脚本路径不是提问，判定是不是交互窗口必须与取 `--session` 一样跳过这一段（见 [会话扫描知识库](SESSION_SCANNING_KNOWLEDGE_BASE.md) §6）。标题生成带 `--no-session`，不得当作用户会话判活。回归：`PiScanTests.test_live_flags_*`。
- 已经跑着、没钉 `--session-id` 的旧占位卡：优先按 `corral-<ident>/` 目录把 keepalive 迁到对应真实卡（`_claim_unique_hosted_newcomer`）。匹配不到时仍仅当该 cwd 里活着的占位卡与本轮新出现的真实卡都恰好一条才认领；无 path 的两张新卡对不上就放弃，避免串台。
...
- **Cursor Task/subagent 会话必须过滤，不能当作用户发起的顶层 chat 列出。** Cursor 多智能体任务会为每个 subagent 在 `~/.cursor/chats/<ws>/<chatId>/` 落独立目录；`meta.json` 带 `isSubagent: true`（即使未来版本补了 `title`/`cwd`/`prompt_history` 也必须跳过）。次级信号在 `store.db` 的 `meta` 表 `subagentInfo.parentAgentId`，但列表扫描阶段不读 store.db，只认 `meta.isSubagent`。
...
- 配方从 GitHub 自动生成的**源码归档**（`/archive/refs/tags/vX.Y.Z.tar.gz`）安装，不依赖 Release 附件——这也是 Homebrew 这条路能完全不等 CI 的原因。代价是该归档的 sha256 理论上由 GitHub 的打包实现决定：2017 与 2023 各出过一次全局 checksum 变动（2023-02 后 GitHub 承诺变更前提前半年通知）。真出现哈希对不上，是重算配方哈希的问题，不是本项目产物损坏。
