...
- **tmux 配置内容内联在 `keepalive.py` 的 `_TMUX_CONFIG` 字符串常量里，不是仓库里一个独立的 `.conf` 文件**——这是踩过坑之后改的：独立配置曾未进入安装产物，源码目录能跑、真实安装后却找不到。改成 `_ensure_config_file()` 在每次 `wrap_plan` 时把内联字符串落盘到 `~/.cache/corral/keepalive.tmux.conf`（内容变了才重写，同 `titles.py` 缓存目录）。改配置内容只改 `_TMUX_CONFIG` 常量，不要再新建独立文件；改完实际安装 wheel 到临时目录，确认没有引入新的包外文件依赖。
...
- 打包元数据只维护 `pyproject.toml` 与 Rust `Cargo.toml` 的必要版本同步；构建后端是 Maturin，原生模块使用 Python 3.10 稳定 ABI。**不要**再引入 `setup.cfg` 双源。控制台入口走轻量 `corral.bootstrap:main`，避免版本和只读子命令加载完整界面。
