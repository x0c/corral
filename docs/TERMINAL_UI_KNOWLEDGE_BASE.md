...
| 活跃会话看板 | 侧栏固定入口，对外文案是 Active sessions / 活跃会话；有成员时写成「名称 · N 个会话」，禁止页码 1/1。自动铺当前需要盯的托管会话（等回话 / 干活 / 未读，以及「刚刚」3 分钟内还有真实活动的无信号会话，排序在三者之后），格子里可直接打字；刚结束的当前页成员先暂留再撤 | `activity_board.py` + 侧栏冻项；跨项目；不写水果组；超额仍用 `[` / `]` 翻页，页外等待用黄点；点具体会话即离开；别的窗口自开会话不进格 |
...
- 两到四个会话以分屏打开后立即形成会话组；组名在创建时从水果表随机生成，例如 `Group Apple`、`Group Pineapple`，之后保持不变。未置顶区的组与独立会话顺序跟 `SessionStore` 稳定顺序走——进入 pickup 后已有项位置固定，不会因成员 mtime 更新而上下飘；**新出现**的会话仍由 store 插到最前（可把更旧的组顶下去）。只有置顶的组/会话才排在列表最上（侧栏选中时按 `p`，任意焦点按 `Ctrl+P`，与全文搜索同级的全局键）。
...
- **AI 易错点**【活动看板不是水果组，也不是把置顶项冻回固定头】侧栏固定头第二项「活跃会话 / Active sessions」自动铺当前需要盯的**本窗口托管**会话（等回话 / 干活 / 未读；另加「刚刚」档：最近真实活动在 `display.JUST_NOW_SECONDS` 同一条 3 分钟界内、但没有任何待办信号的托管会话也进格，排序在未读之后，用的是与会话卡「刚刚」文案同一时间源 `mtime`）。入口文案有成员时是「名称 · N 个会话」（单数 `1 session` / `1 个会话`），**禁止再写页码 1/1**——页码不是用户要盯的信息；超额仍用 `[` / `]` 翻页，当前页外还有人在等时入口亮黄点。禁止 `set_group`；关格只是本轮 dismiss，且必须清掉打字钉，否则焦点还在那一格时会弹回。别的窗口自开的会话没有实时画面，不进格。当前页满了新急件排后面，不插队、不自动翻页；后页空位**禁止**按「当前队列下标」切——新急件插到队头后前页成员会整体后移，看起来像被补进本页。正在看的那一格即使不够格也留着；解析焦点键时**不要**要求格子还活着，否则会话刚结束就会丢掉钉住、格子当场消失。当前页其余刚不够格的成员先暂留约 30 秒再撤（到期必须再铺一次，不能只等后台重扫有变化），避免用户还在看收尾时整页抖动；显式关格 / 离开看板 / 翻页仍立刻拿掉。点侧栏某条具体会话 = 离开看板。看板铺满时顶栏加助手进队列，不要报「分屏已满」；顶栏开终端要先离开看板，不能把终端格加进看板再被刷新冲掉。**列表虚拟下标 `1` 现在是看板不是第一条会话**，筛选框无选中时 Down/Enter 的回落必须跳过全部冻项（`STICKY_IDS`），禁止写死 `index = 1`。回归：`test_activity_board.py`（含翻页后队列插队、刚结束暂留、侧栏文案用会话数不用页码）、`test_activity_board_sticky_shows_empty_and_does_not_save_group`、`test_activity_board_lays_out_members_without_saving_group`、`test_activity_board_full_does_not_block_top_bar_new_session`、`test_shell_from_activity_board_leaves_board`。
