...
- **AI 易错点**【筛选状态单一来源】筛选项目只认 `NavState.project_query`。搜索框输入、列表渲染、页头数量和新建会话目录推导必须共用它；不要在列表或弹窗另存一份筛选值。例外且只读：全文搜索弹窗与新建会话弹窗左栏筛选都可把 `project_query` 当初始查询带进去，但自己的查询串不写回这份状态。**新建会话弹窗还须在 `on_mount` 末尾按筛选框现值再 `_rebuild_projects` 一次**：Input 挂载时可能先派发空串 `Changed` 把 `__init__` 里已按初值收窄的列表冲成全量（发版门禁曾因此红）。回归：`test_new_session_modal_initial_query_seeds_filter`。
