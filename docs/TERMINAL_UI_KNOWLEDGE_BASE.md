...
| Footer 操作 | 页面底部可发现的快捷操作提示 | Textual `Footer`（`CorralFooter`）从 `MainScreen.BINDINGS` 读取文案；右端常驻 `vX.Y.Z`。`Ctrl+\` 回列表、`Ctrl+Shift+B` 显隐侧栏仍绑着但 **Footer 永不展示**（点按界面已有同一条路，2026-09-12）。框架自带的命令面板已关闭，`Ctrl+P` 改作全局置顶，`Ctrl+N` 改作全局新建，`Ctrl+A` 改作全局高级操作 |
...
| 主屏布局与 Footer | `ui/main_screen.py`、`ui/footer.py` | `MainScreen.compose()`、`CorralFooter`、`_main_bindings()` | 左栏搜索和列表、右栏、Footer 的唯一组合处；底栏右端常驻 `vX.Y.Z`；`ENABLE_COMMAND_PALETTE = False`，`Ctrl+P` 走置顶、`Ctrl+N` 走新建 |
