...
| 活跃会话看板 | 侧栏固定入口，对外文案是 Active sessions / 活跃会话；有成员时写成「名称 · N 个会话」。入口固定三行（高 3）：首行名称与可选黄点；第二行多于一页时画出可点的「上一页 / 下一页」（中间 `1/2` 只提示位置），循环翻页；第三行留白。只有一页时第二行也留空（禁止 1/1）。底栏在列表持焦时展示 `[` 上一页 / `]` 下一页（循环故两侧都露）。自动铺当前需要盯的**本窗口托管**会话；成员资格与侧栏关注圆点**同源**：仅等回话黄 / 干活绿 / 未读红（`attention.ATTENTION_MARKER_KINDS`），**禁止**再把「刚刚」无信号会话算进活跃人数或候选队列。格子里可直接打字；观看期间成员不主动撤，只要仍被托管就钉在原格，但角标人数只计仍带关注圆点的成员，不得因钉住已消退的格而虚高 | `activity_board.py` + 侧栏冻项；跨项目；不写水果组；超额用第二行「上一页 / 下一页」和 `[` / `]` 翻页，**禁止**依赖 Page Up / Page Down（Mac 笔记本没有这两个键，终端里 Fn+方向键常被终端滚动吃掉，且已用于右栏对话预览）；页外等待用黄点；点具体会话即离开；别的窗口自开会话不进格。**排查「Active sessions 人数和带圆点会话对不上 / 活跃会话比圆点多 / 圆点有遗漏」也读本节与 §6**。**排查「活跃会话看不到翻页 / 没有分页按钮 / Mac 没有 PageUp PageDown / 不知道怎么翻页」也读本节与 §6**。**排查「两格分屏但每格画面只占一半 / 像被压成 1/4 / 分屏里 Claude 只占约 1/3、右侧大块空白」**：较窄观看方（含本看板、控制通道默认 80 列）不得压窄共享画面，见 [内嵌实时终端知识库](EMBEDDED_TERMINAL_KNOWLEDGE_BASE.md) §6 多开窗口条 |
...
| 会话关注状态 | 左栏首行最左用单个圆点提示下一步是否需要用户关注 | 等待回答黄 > 执行中绿 > 未读新结果红 > 无；详情头同步写出状态，不只靠颜色；不等于标题模块或机器接口的业务状态标签。**与「Active sessions · N」强一致**（2026-09-06 裁定）：有圆点的会话才算活跃；两边必须复用同一套判定（`attention.has_attention_marker` / `ATTENTION_MARKER_KINDS`），禁止看板另开「刚刚活跃」旁路，禁止角标把已消退钉住格算进去 |
