...
- `p` 切换置顶：独立会话可单独置顶，会话组只能整体置顶，组内单个成员不能脱离组单独置顶。置顶块排在普通会话组之前，多个置顶项按最近置顶时间排序；组卡与子项始终是不可拆散的一块。**置顶块与未置顶块都非空时**，中间插入一行 `$primary` 冷蓝横线（`PinSeparatorCard`，高 1、居中标签 `Pinned`/`置顶`、`disabled`，键盘 ↑↓ 跳过；无置顶或筛完只剩置顶时不画）。未置顶区只切 **today / older 两桶**（滚动 24 小时，与时间行 `today` 档同一条界）：按 store 稳定顺序扫一遍，桶内相对顺序不变，再拼成 `today + [Today 线] + older`；Today 线仅当两侧都有可见项时出现。会话组不可拆——任一成员 `live` 或 mtime 落在 24h 内，整组进 today。分隔线是区尾（标签标明上面这一段），**禁止 Older/其他** 标签，避免把下方会话说成次要。
