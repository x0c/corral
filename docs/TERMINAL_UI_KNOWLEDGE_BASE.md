...
- **AI 易错点**【侧栏突然多一张「cli 生成 s0」且点开没有消息 → 不是标题生成去调了 Claude，也不是对话解析丢了正文】**这是空壳占位卡。** 2026-09-12 真机：项目名 `cli` 来自测试工作目录，标题 `生成s0` 来自真实标题缓存里被单测写脏的 `claude:s0`，卡片本身是保活通道里界面测试留下的托管窗格（无历史路径、无用户消息、`provisional`）。点开当然没有对话预览——没有可预览的历史。现标题通道是独立网关，不经过 Claude；旧「哪个助手最近跑通就用哪个」的命令行轮转已退役。同类残留还会以 `New Claude session` + 短编号（如 `directli`）出现。不要为消这张卡去改标题 prompt 或 Claude 扫描过滤。收掉测试托管、清掉缓存里的 `claude:s0`…`s29` 测试键即可；机制见 [会话扫描知识库](SESSION_SCANNING_KNOWLEDGE_BASE.md) §6、[维护指南](MAINTAINER_GUIDE.md)「标题与排序」。
