...
| 按键绑定 | 主操作 | `MainScreen.BINDINGS`、`_main_bindings()` | 全局：`Ctrl+F` 搜索、`Ctrl+P` 置顶、`Ctrl+N` 新建、`Ctrl+A` 高级操作（均 `priority=True`）；列表侧 `a`/`q`/`x`；`c` 关格、`Ctrl+\` 回列表不上 Footer。**不绑侧栏显隐**（只点顶栏 ◀/▶，2026-09-12）。`Ctrl+G` 小窗、F12 截图不上 Footer |
