...
- **展开态（默认）**：列出**全部**真人提问（不再砍中间），每条最多两行（`_MAX_PROMPT_LINES`），超出末行加 `...`；续行缩进与首行正文同一列（时间列固定 7 格）。浮层仍有高度上限，多出来的提问靠区域内滚动查看——**禁止**再画「中间省略 N 条」或静默丢掉中间提问。浮层底色是格子高光的一部分：选中 / 持焦格 `$pane-active-background`（蓝条），没选中 `$surface`（跟没有高光的顶底条同色）。提问块按奇偶叠一层 40% `$primary` 斑马纹（从**当前**浮层底混合，禁止叠 `$foreground`——那是灰，会把激活条的蓝洗脏），页眉 / 页脚不涂；hover 切 `$primary-muted` 时条纹跟着走。
