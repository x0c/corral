...
- 侧边栏斑马纹按**块**交替，不是按卡片：独立会话一块，会话组（组卡 + 全部成员）一块，块内同色。`＋ 新建` 与置顶分隔线不参与、不计入相位；分隔线之后相位重置，两区都以无条纹起头。条纹画在 `SessionCard` / `SessionGroupCard` 上（`$foreground 4%` 半透明），叠在列表底 / 选中 / 分屏底色之上。
...
- **AI 易错点**【斑马纹是块级、画在卡片层、必须半透明】相位按块交替：独立会话一块，会话组（组卡 + 全部成员）一块；`＋ 新建` 与置顶分隔线不参与；分隔线之后重置，避免置顶/展开收起把下方整列相位翻转。类标在 `SessionCard` / `SessionGroupCard` 上，背景用 `$foreground 4%`——Textual 子类 `DEFAULT_CSS` 的 tie-breaker 高于基类，写到 `ListItem.-stripe` 会压过 `ListView` 自带的 `.-highlight`，把选中底色吃掉。失焦选中色 `block-cursor-blurred-background` 必须够不透明（约 `CC`），否则 4% 条纹会把它压平，焦点在右栏时分不清哪行是选中。全量重建与原地更新都要 `_apply_stripes()`。回归：`SidebarStripeTests`。
