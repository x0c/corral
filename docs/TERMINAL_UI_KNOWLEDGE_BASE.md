...
| 活跃会话看板 | 侧栏固定入口，对外文案是 Active sessions / 活跃会话；有成员时写成「名称 · N 个会话」，禁止页码 1/1。自动铺当前需要盯的托管会话（等回话 / 干活 / 未读），格子里可直接打字；刚结束的当前页成员先暂留再撤 | `activity_board.py` + 侧栏冻项；跨项目；不写水果组；超额仍用 `[` / `]` 翻页，页外等待用黄点；点具体会话即离开；别的窗口自开会话不进格 |
