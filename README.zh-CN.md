...
- 最上两行永远钉住不滚：第一行仍是「＋ 新建会话」（英文 `+ New session`），回车选项目和助手、空白会话直接托管在右栏。它下面是「活动会话」：自动铺最多四格当前需要盯的托管会话（等你回话、正在干活、刚出结果还没看）。别的窗口里自己开的会话没有实时画面，不进格子。人多了用 `[` / `]` 翻页，当前页外还有人在等时入口上会亮黄点。点侧栏某条具体会话即离开看板，回到原来的单会话或手动分屏。看板不会写成水果名会话组。
...
| `Enter` | 用原生助手恢复选中的会话（若已在后台运行则重新接上）；在固定的「＋ 新建会话」上进入新建流程；在「活动会话」上打开动态看板 |
| `a` | 打开高级接力操作 |
