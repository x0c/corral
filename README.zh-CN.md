...
如果想换助手继续：打开会话，按 `Ctrl+A`，选择助手。例如 Claude 完成初步实现后，可以把这段对话交给 Codex 新开会话检查。源对话保持完整，接手的助手自行读取所需历史。要用原助手原生恢复，按 `Enter` 即可。
