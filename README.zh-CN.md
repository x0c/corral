...
高级操作（`a`）菜单：第一项**导出会话**写出与 `pickup share` 相同的 transcript 并把文件路径复制到剪贴板；第二项**复制会话**在旁边克隆当前对话；选某个助手则一律**新建会话**并读取源历史——选其他助手或选同一个助手都可以（原会话卡住 / 出 bug 时用同助手另起）。接力新会话默认出现在被接力会话旁边。提示词中包含：
