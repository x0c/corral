...
| --- | --- |
| `Up` / `Down` / `j` / `k` | 移动选中项 |
| `/` | 聚焦侧边栏筛选框（对项目名、路径和会话标题做大小写无关的模糊匹配） |
...
界面每次启动时都会在后台检查是否有新版本（一次对公开 GitHub API 的 HTTPS 请求，见[隐私模型](#隐私模型)）。如果你的安装方式支持就地升级（Homebrew tap 或基于 `pip` 的安装），右下角会出现一个小提示；点它即可更新，然后可以顺手就地重启 `pickup`。忽略当天的提示只需一次点击；如果你仍未更新，第二天它会再出现。源码／开发检出永远不会被打扰——这种安装路径下检查会被完全跳过。
...
| `src/pickup/split_layout.py` | 记住的活跃分屏组合 |
