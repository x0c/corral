...
- 底部 Textual `Footer` 显示 `^n New` / `^n 新建`，以及列表持焦时的 `a Advanced` / `a 高级操作` 与 `^a Advanced` / `^a 高级操作`（右栏对着某个会话时仍保留 `^a`，筛选框里不抢 `Ctrl+A`）。**不得出现** `Back to list` / `返回列表`、`Toggle sidebar` / `显隐侧栏`（键仍可用，只是不上底栏）。
