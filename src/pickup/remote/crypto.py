...
握手（每条连接一次，双方各出一把临时密钥，兼顾前向保密与身份认证）：
    ...
    key_d2h = HKDF(ikm, salt, "pickup/remote/v1 d2h")
    key_h2d = HKDF(ikm, salt, "pickup/remote/v1 h2d")
...
_INFO_D2H = b"pickup/remote/v1 d2h"
_INFO_H2D = b"pickup/remote/v1 h2d"
_KEY_LEN = 32
_NONCE_LEN = 12
_MAX_COUNTER = (1 << 64) - 1

_MISSING_DEPS_HINT = (
    "手机端接力需要额外的加密与网络组件。请执行：pip install 'pickup[remote]'\n"
...
_INFO_PUSH = b"pickup/remote/v1 push"
