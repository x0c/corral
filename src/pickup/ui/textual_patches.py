...
在 ``PickupApp`` 导入时安装一次。补丁必须幂等：测试与多次 import 不得叠
多层包装。
"""
...
def _patch_lru_cache_eviction() -> None:
    ...
    if getattr(LRUCache.set, "_pickup_safe_eviction", False):
        return
    ...
    set_safe._pickup_safe_eviction = True  # type: ignore[attr-defined]
