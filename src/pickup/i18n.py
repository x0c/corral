...
_MESSAGES: dict[str, dict[str, str]] = {
    ...
    "error.tmux_missing": {
        "en": (
            "pickup needs tmux to run. Install it first "
    ...
    "error.tmux_old": {
        "en": (
            "pickup needs tmux {need} or later (detected {current}): hosted sessions "
        ...
        "zh": (
            "pickup 需要 tmux {need} 及以上版本（当前检测到 {current}）：会话托管依赖 "
    ...
    "remote.cli.epilog": {
        "en": (
            "Common flow:\n"
            "  pickup remote start        # first start prints a pairing QR code\n"
            "  pickup remote pair         # pair another phone\n"
            "  pickup remote pair --readonly  # read-only pairing (no input/delete)\n"
            "  pickup remote status       # check whether it is running\n"
            "  pickup remote rotate-token # rotate the relay registration credential\n"
            "  pickup remote stop         # stop\n"
        ),
        "zh": (
            "常用流程：\n"
    ...
    "remote.help.rotate_token": {
        "en": "Rotate the relay registration credential",
        "zh": "轮换中继注册凭据",
    },
    "remote.help.stop": {
    ...
    "remote.start.already_running": {
        "en": (
            ...
            "To restart, run pickup remote stop first, or add --force to stop the old process then start"
        ),
        "zh": (
            "常驻服务已经在跑了（进程 {pid}）。要重开先执行 pickup remote stop，"
            "或加 --force 先停旧进程再启动"
        ),
    },
    "remote.start.ready": {
    ...
    "remote.pair.scan": {
        "en": "\nScan this code with the pickup phone app to pair{mode_hint}:\n",
    ...
    "remote.pair.service_not_running": {
        "en": (
            ...
            "After scanning, wait until pickup remote start is running before you can connect.\n"
        ),
        "zh": "提示：常驻服务还没启动，扫码后要等 pickup remote start 跑起来才能连上。\n",
    },
    ...
    "remote.devices.empty": {
        "en": "No phones have been paired yet. Run pickup remote pair to generate a QR code.",
        "zh": "还没有配对过任何手机。执行 pickup remote pair 生成二维码。",
    },
    ...
    "remote.rotate.done": {
        "en": (
            "Relay registration credential rotated. Restart the always-on service "
            "(pickup remote stop && pickup remote start) for the new credential to take "
    ...
    "shim.status.outdated": {
        "en": "Needs update (run pickup shim install again)",
        "zh": "需要更新（重新执行 pickup shim install）",
    },
    ...
    "shim.err.usage_hint": {
        "en": "Run pickup shim --help for usage",
        "zh": "运行 pickup shim --help 查看用法",
    },
    ...
    "shim.cli.description": {
        "en": "Automatically route typed claude/codex/… commands through pickup hosted launch",
        "zh": "把手敲的 claude/codex/… 自动改走 pickup 托管启动",
    },
    "shim.cli.help.shell": {
    ...
    "cli.help.description": {
        "en": (
            "pickup: a terminal session handoff tool.\n"
    ...
    "cli.help.epilog": {
        "en": (
            "Examples:\n"
            "  pickup                 # Start the TUI, pick a session interactively, and take over the terminal\n"
            "  pickup --json          # Print a JSON session list and exit without starting the TUI (legacy format)\n"
            "  pickup --json --limit 5  # JSON mode, at most 5 sessions per runtime\n"
            "  pickup describe        # Show usage for list/search/show/context and other subcommands\n"
            "  pickup shim status     # Inspect/install command intercept "
            "so typing claude/codex/… is routed through pickup\n"
            "\n"
            "JSON output fields:\n"
    ...
    "cli.help.debug": {
        "en": "Enable verbose diagnostic logs; you can also set PICKUP_DEBUG=1",
        "zh": "启用详细诊断日志，也可设置 PICKUP_DEBUG=1",
    },
    "cli.help.quiet": {
