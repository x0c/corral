...
粘贴事件转发给 tmux」，取代旧版 corral.py 里的 _draw_embed_pane + 主循环中一大段
手写 SGR 鼠标解析。
...
滚动/选词说明：tmux 回滚（history_offset）与 corral 主屏的按键滚动完全保留；
...
# 等尺寸停稳再做一次（与 CorralApp 整屏重绘防抖同量级，避免拖动期狂刷）。
...
class EmbedPane(Widget):
    ...
    def __init__(
        ...
        *,
        ...
        # 尺寸，作为首帧前提示；真实 Resize 抵达或 focus_session(target_size=None)
        # 时清除。稳态抓帧优先用 pane_state 回读的真实列数。
        self._capture_size_override: tuple[int, int] | None = None
        # pane_state 回读的 tmux 真实 (宽, 高)。抓帧解析用它，避免按控件宽度
        # 去解析一个仍停在旧列数的窗口。
        self._tmux_pane_size: tuple[int, int] | None = None
    ...
    def on_mount(self) -> None:
        ...
        bg = corral._background_rgb(self._osc_report)
        if bg is not None:
    ...
    def update_terminal_background(self, osc_report: bytes) -> None:
        ...
        self._osc_report = osc_report
        bg = corral._background_rgb(osc_report)
        if bg is not None:
            self.styles.background = bg
        if self.session_name:
    ...
    def _capture_size(self) -> tuple[int, int]:
        ...
        格数变化的空档优先用预测尺寸（tmux 可能已经按新宽 resize，控件还是旧宽）。
        布局落定后改用 pane_state 回读的真实列数，避免把仍停在窄窗里的画面按格宽
        解析出一排空白。两者都没有时才退回控件尺寸。
        """
        if self._capture_size_override is not None:
            return self._capture_size_override
    ...
    def _capture_loop(self) -> None:
        ...
        misses = 0
        last_capture = 0.0
        last_state = None      # pane_state 降频缓存：前 6 项交互 + 可选 (宽, 高)
        last_state_at = 0.0
        while not self._stop.is_set():
            try:
                ...
                corral._log_embed_error("Textual 抓帧线程", exc)
                last_frame_key = None
                last_state = None
                last_state_at = 0.0
    ...
    async def _on_key(self, event: events.Key) -> None:
        ...
        if event.key == "ctrl+shift+b":
            ...
            # corral 截走会把侧栏藏起来；换 Ctrl+Shift+B 后互不干扰（2026-08-04）。
            event.stop()
            event.prevent_default()
    ...
    def _on_resize(self, event: events.Resize) -> None:
        ...
        # 布局已经落地：预测尺寸过期。不能要求「完全相等才清」——预测余数分配
        # 曾与 Textual 实际布局差 1 列，中间格的 override 会永远赖着，后续变宽
        # 仍按旧窄宽解析。
        self._capture_size_override = None
