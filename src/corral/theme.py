...
def _probe_osc_colours(timeout: float = 1.2) -> bytes | None:
    ...
    corral 自己跑在 tmux 里时，学 Claude Code 的做法同时发 DCS passthrough 包装
    的查询——外层 tmux 开 allow-passthrough 时可穿透直达真实终端；裸查询部分由
    外层 tmux 用其 client 缓存值应答（3.4+）。非 TTY、终端不应答（超时）时返回
    None。测试钩子：CORRAL_OSC_REPORT（hex 编码）。
    """
    hook = getenv("OSC_REPORT") or ""
    if hook:
        try:
    ...
    # 之后、漏进随后接管的 Textual——直启（`corral cursor` 等）默认焦点在搜索框
...
def _split_osc_report(osc_report: bytes | None) -> tuple[bytes | None, bytes | None]:
    ...
    **必须拆开**：tmux 的 `refresh-client -r <pane>:<应答>` 只解析参数里的**第一条**
    ...
    浅色终端上前景恰好是黑色，症状最明显：corral 自身正确切浅色，内嵌 agent 全是深色。
