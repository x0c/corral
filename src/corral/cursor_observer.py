...
观察器只管理 Cursor 用户级 ``hooks.json`` 中由 corral 创建的条目。配置损坏、
...
class _ArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> None:
        raise ObserverError(
            ...
            hint="运行 corral observer --help 查看用法",
        )
...
def _hook_command() -> str:
    argv = [sys.executable, "-m", "corral", "_cursor-hook"]
    if os.name == "nt":
...
def install(
    ...
    """幂等安装或升级 corral 管理的 Cursor 观察条目。"""
...
def _parser() -> _ArgumentParser:
    parser = _ArgumentParser(prog="corral observer", add_help=True)
...
def cli_main(argv: Sequence[str] | None = None) -> int:
    """处理 ``corral observer ...``；非 TTY 自动输出 JSON envelope。"""
