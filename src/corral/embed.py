"""内嵌宿主：把托管在 tmux 里的会话画面搬进 corral 的 Textual 右栏。
...
from corral import keepalive, liveness
...
# 知识库与既有测试仍用这对模块级名字。
_IMG_SENTINEL_BEGIN = IMG_SENTINEL_BEGIN
_IMG_SENTINEL_END = IMG_SENTINEL_END

_CREATE_TIMEOUT = 5.0
_CALL_TIMEOUT = 1.5

_PASTE_BUFFER = "corral-embed"
...
def available(disabled_flag: bool = False) -> bool:
    """内嵌可用性：tmux 已安装且未被显式禁用（--no-keepalive / CORRAL_KEEPALIVE=0，
    过渡名 PICKUP_KEEPALIVE=0 与更旧名 SC_KEEPALIVE=0 同样生效）。
...
def host_session(
    ...
    真机排查记录（这条限制目前无法从 corral 这一侧完全消除，如实记录）：
...
def resize(name: str, width: int, height: int) -> None:
    ...
    try:
        subprocess.run(
            [*keepalive.tmux_argv(name), "resize-window", "-t", name,
...
_IMAGE_PASTE_DIR = ".corral-pastes"
...
def extract_pasted_image(text: str) -> bytes | None:
    ...
    """
    payload = image_sentinel_payload(text)
    if payload is None:
        return None
...
def save_image_and_paste_path(name: str, image_bytes: bytes) -> str | None:
    ...
    base_dir = (
        ...
        else os.path.join(tempfile.gettempdir(), "corral-pastes")
    )
    try:
...
# ---- tmux 版本探测：硬性最低版本（corral._require_tmux）+ 背景色注入软性检查 ----

# `new-session -e` 环境变量注入（CORRAL_RUNTIME/CORRAL_SESSION_ID 等托管元数据的
# 唯一注入点）与 pause-after 流控通知（%pause）都要求 tmux 3.2+（2021-04 发布）；
# 更旧的版本上 host_session()/wrap_plan() 会在创建会话时报一个笼统的
# EmbedError，看不出是版本问题。corral._require_tmux() 用这个常量做硬性拦截。
